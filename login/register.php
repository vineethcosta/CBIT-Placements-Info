...
if (isset($_POST['signup'])) {
    ...
    if ($count == 0) { // if email is not found add user
        ...
        $stmts = $conn->prepare("INSERT INTO users(username,email,password,branch,attendance,cgpa) VALUES(?, ?, ? ,? ,? ,?)");
        $stmts->bind_param("ssssdd", $uname, $email, $password, $ubranch ,$uatt, $ucgpa);
